  ...
  return (
        <div className="p-4 sm:p-8 relative">
            ...
            {projects.map((project, index) => (
                <PixelCard key={index} variant={project.color} gap={15} className="hover:cursor-pointer hover:scale-105 transition-transform duration-400 ease-in-out border-2  bg-charcoal border-amber-500">
                <div className="absolute top-4 text-lg text-amber-500 font-bold font-aldrich capitalize">{project.name}</div>
                <div className="absolute text-sm text-center text-justify p-5 text-white font-bold font-aldrich capitalize">{project.description}</div>
                <a
                    href={project.link}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="absolute bottom-4 bg-navy border-powder border-2 text-white px-4 py-2 rounded-lg text-sm hover:bg-powder hover:text-black font-poppins hover:font-semibold transition-all duration-500 ease-in-out"
                >
                    View Website
                </a>
                </PixelCard>
